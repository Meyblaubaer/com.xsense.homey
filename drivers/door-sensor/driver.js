...
class DoorSensorDriver extends Homey.Driver {
  ...
  async onPairListDevices(session) {
    ...
    try {
      ...
      // Supported types: SDS0A, SES01, XDS01, and any containing 'DOOR' or 'WINDOW' or 'CONTACT'
      for (const device of data.devices) {
        const deviceType = (device.type || device.deviceType || '').toUpperCase();

        // Supported types: SDS0A, SES01, XDS01
        const doorTypes = ['SDS0A', 'SES01', 'XDS01', 'DOOR', 'WINDOW', 'CONTACT'];
        if (!doorTypes.some(t => deviceType.includes(t))) {
          this.log(`Skipping device ${device.deviceName || device.name} (Type: ${deviceType}) - Not a door/window sensor`);
          continue;
        }
