...
class MotionSensorDriver extends Homey.Driver {
  ...
  async onPairListDevices(session) {
    ...
    try {
      ...
      // Supported types: SMS0A, SMS01, XMS01, and any containing 'MOTION' or 'PIR'
      for (const device of data.devices) {
        const deviceType = (device.type || device.deviceType || '').toUpperCase();

        // Supported types: SMS0A, SMS01, XMS01
        const motionTypes = ['SMS0A', 'SMS01', 'XMS01', 'MOTION', 'PIR'];
        if (!motionTypes.some(t => deviceType.includes(t))) {
          this.log(`Skipping device ${device.deviceName || device.name} (Type: ${deviceType}) - Not a motion sensor`);
          continue;
        }
