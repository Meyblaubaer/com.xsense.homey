...
class DebugLogger {
  constructor(homey, context = 'XSense') {
    ...
    if (this.debugEnabled.any) {
      try {
        ...
        console.error('[DebugLogger] Failed to create debug directory:', err);
      }
    }
  ...
  log(...args) {
    ...
    console.log(`[${timestamp}] [${this.context}]`, ...args);
  }
  ...
  _writeToFile(filename, content, append = false) {
    ...
    try {
      ...
      console.error(`[DebugLogger] Failed to write ${filename}:`, err);
    }
  }
