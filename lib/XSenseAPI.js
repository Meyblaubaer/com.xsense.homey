  ...
  async _apiCall(bizCode, params = {}, unauth = false) {
    ...
    try {
      ...
      if (errorCode !== undefined && errorCode !== 200 && errorCode !== 0) {
        ...
        if (['10000008', '10000020'].includes(String(errorCode)) || errorMsg.includes('another device is logged in')) {
    ...
    for (const house of houses) {
      ...
      for (const station of stations) {
        ...
        if (station.category === 'SC07-WX') {
          console.log(`[XSenseAPI] Found SC07-WX WiFi Device (as Station): ${station.stationName}`);
