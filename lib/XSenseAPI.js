  ...
  async getAllDevices() {
    ...
    for (const house of houses) {
      ...
      for (const stationData of processedStations) {
        ...
        if (stationData.category === 'SC07-WX') {
          console.log(`[XSenseAPI] Found SC07-WX WiFi Device (as Station): ${stationData.stationName}`);

          // Map station shadow to device-like structure
          const wifiDevice = {
            id: stationData.stationId, // Use stationId as deviceId
            ...
            // Map station properties to device properties
            deviceName: stationData.stationName,
            deviceType: 'SC07-WX', // Explicitly set type
            deviceSn: stationData.stationSn,

            // Map status from station shadow/properties
            onLine: stationData.onLine,
            wifiRssi: stationData.wifiRSSI, // from shadow (capitalization varies, handled in driver usually)

            // Create 'dummy' shadow data if needed, or map directly
            // The driver expects 'alarmStatus', 'batteryLevel' etc.
            // We need to see what the SC07-WX shadow provides.
            // Based on logs: {"type": "SC07-WX", "stationSN": "...", "wifiRSSI": "-45"}
            // It might be reporting status in '2nd_systime' or another shadow.
            // For now, we pass everything we have.
            ...stationData
          };
