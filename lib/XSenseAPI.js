...
class XSenseAPI {
  ...
  async _apiCall(bizCode, params = {}, unauth = false) {
    ...
    try {
      ...
      if (!response.ok) {
        const text = await response.text();
        this.debug.error(`[XSenseAPI] Error response:`, text);
      ...
      if (errorCode !== undefined && errorCode !== 200 && errorCode !== 0) {
        ...
        if (['10000008', '10000020', '10000004'].includes(String(errorCode)) || errorMsg.includes('another device is logged in') || errorMsg.includes('Authorization cannot be empty')) {
          this.debug.log('[WARN]', `[XSenseAPI] Session invalid (Code: ${errorCode}, Msg: ${errorMsg}). Invalidating session.`);
  ...
  _handleWiFiDeviceShadow(topic, data) {
    ...
    if (reported.devs) {
      ...
      for (const [deviceSn, deviceData] of Object.entries(reported.devs)) {
        const deviceId = this.devicesBySn.get(deviceSn);
        if (!deviceId) {
          // console.warn(`[XSenseAPI] Device ${deviceSn} in devs map not found in cache`);
          continue;
        }
        ...
        console.log(`[XSenseAPI] Updated device ${deviceSn} from mainpage devs map (batInfo: ${merged.batInfo})`);
      }
